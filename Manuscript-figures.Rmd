...
# Responders as fraction of total reads
```{r}
...
# Table summarize composition of responders by phylum:
df.phylum <- subset(df, select = c(mu.survival.max, mu.fast.max, mu.affinity.max, Phylum)) %>% unique()

df.phylum %>% mutate(Phylum = as.factor(Phylum)) %>% subset(mu.affinity.max>0) %>%subset(select = c(Phylum)) %>% View()

df.Class <- subset(df, select = c(mu.survival.max, mu.fast.max, mu.affinity.max, Class)) %>% unique()

df.Class %>% mutate(Class = as.factor(Class)) %>% subset(mu.fast.max>0) %>%subset(select = c(Class)) %>% summary()



# Count number of taxa by sample:
# Create a factor corresponding to the Genera
genfac = factor(tax_table(ps.norm.cDNA)[, "Genus"])
# Tabulate the counts for each genera in each sample
gentab = apply(otu_table(ps.norm.cDNA), MARGIN = 2, function(x) {
    tapply(x, INDEX = genfac, FUN = sum, na.rm = TRUE, simplify = TRUE)
})
head(gentab)[, 1:10]

x <- as.data.frame(gentab)
```

# New figures: 
```{r}
### BC dissim of burned vs. unburned on the x axis with a respiration metric on the y axis

# Import BC dissimilarity data
df.distances <- read.csv('../../data/bray-curtis-dissimilarites.csv')

# Combine this with metadata
df.distances <- merge(df.distances, subset(df.meta, burn.trtmt != 'control'))    
...
df.resp <- read.csv("../../data/fractional-c-remaining.csv")


df.merge <- merge(df.distances, df.resp, by = c('core.id','incub.trtmt')) %>%
  subset(incub.trtmt == 'SI') %>%
  subset(DayFactor == 34) %>%
  mutate(mass.C.respired = mass.C.remaining.g * (1-fractional.C.remaining))

pBC = ggplot(df.merge, aes(x=BC.distance.btw.burned.and.control, y=k1, color = horizon)) + 
  geom_point() + 
  scale_color_manual(values = c('darkgreen','grey70'))+
  stat_smooth(method=lm,se = FALSE,color='black') +
  theme_bw() + 
  #facet_wrap(~horizon, ncol=2,scales = 'fixed',
   #          labeller = labeller(horizon = HorizonLabels))+
  labs(y = 'Decay rate constant for \nfast C pool (k1)',
       x = 'Bray-Curtis dissimilarity \ncompared to unburned control')


summary(lm(data=df.merge,k1~BC.distance.btw.burned.and.control))




### Ordination colored by respiration metric:
# Full ordination
ps.norm.SI <- prune_samples(sample_data(ps.norm.full)$incub.trtmt =='SI', ps.norm.full)
ps.norm.SI <- prune_taxa(taxa_sums(ps.norm.SI)>0, ps.norm.SI)

ord.nmds = ordinate(ps.norm.SI,method = "NMDS",ddistance="wunifrac",k=3, weighted=TRUE)

# quick look at results:
plot_ordination(ps.norm.SI,ord.nmds,color="burn.trtmt",shape="incub.trtmt")
colnames(sample_data(ps.norm))



# Ordination plot with horizons and deg.hrs
p1 = plot_ordination(ps.norm.SI, ord.nmds, axes = c(1,2), color = "",
                      shape = 'burn.trtmt') + 
  ...
  scale_shape_manual(values = c('circle','square','triangle'),
                     labels = c('Unburned control', 'Moist soil','Dry soil'),
                     breaks = c('control','wet','dry')) +
...
## What fraction of total reads in dry soil burns 24 hr post burn were bacillus?
#  Bacillus fire surviving taxa = 8f0e2646c1389192d5a822f62bd295cf, 8adb5559e135233565acfea1287280bf
df <- read.csv('../../data/sequence-data/LibCombined/corncob-output/Manuscript/Trait-responder-OTUs.csv')

df.bac <- read.csv('../../data/sequence-data/LibCombined/corncob-output/Manuscript/Trait-responder-OTUs.csv') %>%
  subset(mu.survival.max>0) %>%
  subset(Genus=='Bacillus')
  
ps.norm.bac <- prune_samples(sample_data(ps.norm.gDNA)$incub.trtmt == 'pb', ps.norm.gDNA) 
ps.norm.bac <- prune_taxa(taxa_names(ps.norm.bac) %in% df.bac$OTU, ps.norm.bac)

bac <- psmelt(ps.norm.bac)
# Answer 9% (8f0e2646c1389192d5a822f62bd295cf) and 4% (8adb5559e135233565acfea1287280bf) of total reads in dry soil burns


### What percentage of all OTUs were identified as fast growers?
df.fast <- df %>%
  subset(mu.fast.max>0)

ps.norm.SI <- prune_samples(sample_data(ps.norm.gDNA)$incub.trtmt == 'SI', ps.norm.gDNA) 
ps.norm.SI <- prune_taxa(taxa_sums(ps.norm.SI)>0, ps.norm.SI)
ps.norm.SI
# Answer: IN the 5 week incubation, 0.4%



### Max relative abundance of responding taxa in lab samples?
ps.responders <- prune_taxa(taxa_names(ps.norm.gDNA) %in% df$OTU, ps.norm.gDNA)
df.responders <- psmelt(ps.responders)

df.responders <- df.responders %>%
  subset(select = c(Sample, Abundance, Full.id,burn.trtmt))

Num.responders <- df.responders %>% group_by(Full.id) %>% mutate(abun = sum(Abundance))
max(Num.responders$abun) # = 0.604

Num.responders.in.dry <- df.responders %>%
  subset(burn.trtmt == 'dry') %>% 
  group_by(Full.id) %>% 
  mutate(combined.abun = sum(Abundance))


mean(Num.responders.in.dry$combined.abun) # = 15
sd(Num.responders.in.dry$combined.abun) # = 
...
df <- read.csv('../../data/sequence-data/LibCombined/corncob-output/Manuscript/all-responders.csv')
...
df.survival <- df %>%
  subset(experiment == 'survival'&
           T.cutoff %in% c('Low thermo > 50C','Mid thermo > 50C') & 
           intercept > 0) %>%
  subset(intercept > (mean(intercept)-1*sd(intercept)) ) %>% 
...
df.affinity <- df %>%
  subset(experiment == 'affinity'&
           T.cutoff %in% c('Low thermo > 50','Mid thermo > 50') &
           intercept > 0) %>%
  subset(intercept > (mean(intercept)-1*sd(intercept)) ) %>% 
  subset(select = c(OTU,Phylum, Class, Order, Family, Genus, Species)) %>%
  unique()
...
df.affinity.L2FC  <- df.L2FC %>%
  subset(experiment == 'affinity'  &
           T.cutoff %in% c('low thermo > 50 C','mid thermo > 50 C') &
           mu > 0) %>%
  subset(mu > (mean(mu)-1*sd(mu))) %>%
  subset(select = c(OTU)) %>%
  unique() 
...
#   the mean of the intercept. Remove duplicate taxa. 
...
# SI: plot responders as fraction of total comm. vs. total C mineralized over 5 week incub.
```{r}
### Goal plot with fraction of total comm by trait category vs. total C-CO2 respired after 5 weeks 
...
# assign traits:
for (i in 1:nrow(df)) {
  if (is.na(df$mu.fast.max[i]) & is.na(df$mu.affinity.max[i])) {
    df$trait[i] = 'Survivor' 
  } else if (is.na(df$mu.survival.max[i]) & is.na(df$mu.affinity.max[i])) {
    df$trait[i] = 'Fast grower' 
  } else if (is.na(df$mu.survival.max[i]) & is.na(df$mu.fast.max[i])) {
    df$trait[i] = 'Affinity taxa' 
  } else if (is.na(df$mu.survival.max[i])) {
    df$trait[i] = 'Fast grower and affinity taxa'
  } else if (is.na(df$mu.affinity.max[i])) {
    df$trait[i] = 'Survivor and fast grower'
  } else if (is.na(df$mu.fast.max[i])) {
    df$trait[i] = 'Survivor and affinity taxa' 
  } else {
    df$trait[i] = 'ALL'
  }
}
...
# 2. import full comm data at end of SI incubation
ps.norm.SI <- prune_samples(sample_data(ps.norm.full)$incub.trtmt == 'SI', ps.norm.full)

# 3. Assign traits to OTUs in the SI comm data
ps.norm.SI <- prune_taxa(taxa_names(ps.norm.SI) %in% df$OTU, ps.norm.SI)
ps.norm.SI <- prune_taxa(taxa_sums(ps.norm.SI)>0, ps.norm.SI)

df.norm.SI <- psmelt(ps.norm.SI)

df.norm.SI <- merge(df.norm.SI, subset(df, select = c(OTU, trait)))

p = ggplot(df.norm.SI, aes(x=burn.trtmt, y=Abundance)) + 
  geom_bar(aes(fill=trait), stat='identity', position='stack')

# 4. Import respiration data
#df.resp <- read.csv("../../data/fractional-c-remaining.csv")
# df.resp <- df (imported from "incubations_microbial_respiration_TLWcheck.R")


# 5. sum total respiration at end of 5 weeks
df.resp.SI <- subset(df.resp, Incubation.trtmt == 'SI') 

# 6. merge trait and respiration dataframes
df.sum <- df.norm.SI %>%
  group_by(Full.id, trait) %>%
  mutate(trait.abun = sum(Abundance)) %>%
  subset(select = c(core.id,trait,trait.abun)) %>%
  unique() %>%
  merge(df.resp.SI, by = 'core.id')

# 7. plot fraction of total comm vs. total C-CO2 respired
p = ggplot(subset(df.sum, Day==34), aes(x=trait.abun, y = C.CO2.absorbed.per.g.C)) + 
  geom_point(aes(color=trait)) + 
  facet_wrap(~burn.trtmt)+
  theme_bw() 
  

p = ggplot(df.sum, aes(x=(1-fractional.C.remaining),y=trait.abun)) + 
  geom_bar(aes(fill=trait), stat='identity', position='stack') + 
  theme_bw()


# Initial mineralization after 2-7 days
...
for (i in 1:nrow(df)) {
  if (abs(df$intercept[i]) > 2) {
    df$trimmed.intercept[i] = df$intercept[i]
  } else {
    df$trimmed.intercept[i] = NA
  }
}

p = ggplot(df) + 
  geom_histogram(aes(intercept),fill = 'red', binwidth=1) + 
  #geom_histogram(aes(trimmed.intercept), fill = 'blue', binwidth=1) + 
  theme_bw() + 
...
p1 = ggplot(subset(df, experiment == 'survival'), aes(intercept)) +
  geom_histogram() + 
  theme_bw() + 
  facet_grid(horizon+Comparison~Temp.cutoff, scales = 'free')

p2 = ggplot(subset(df, experiment == 'fast growth'), aes(intercept)) +
  geom_histogram() + 
  theme_bw() + 
  facet_grid(horizon+Comparison~pH.cutoff, scales = 'free')

p3 = ggplot(subset(df, experiment == 'survival'), aes(intercept)) +
  geom_histogram() + 
  theme_bw() + 
  facet_grid(horizon+Comparison~Temp.cutoff, scales = 'free')

length(unique(subset(df, intercept > 0 & experiment == 'affinity')$OTU))
length(unique(subset(df, intercept < 0 & experiment == 'affinity')$OTU))
length(unique(subset(df, intercept > 1 & experiment == 'affinity')$OTU))
length(unique(subset(df, intercept < -1 & experiment == 'affinity')$OTU))
