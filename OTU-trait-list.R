...
setwd('../Box/WhitmanLab/Projects/WoodBuffalo/FireSim2019/data/sequence-data/LibCombined/corncob-output/Manuscript')
...
write.csv('data/sequence-data/LibCombined/corncob-output/Manuscript/All-responders.csv')
...
df.survivors <- merge(df.exp1, df.ratio, by = "OTU") %>%
  ...
  subset(select = c(OTU, comparison, experiment, horizon,
                    ...
                    mu.survival.max, mu.fast.max, mu.affinity.max)) %>% 
  unique()
...
### Step 4. Save output?
write.csv('data/sequence-data/LibCombined/corncob-output/Manuscript/Trait-responder-OTUs.csv')
